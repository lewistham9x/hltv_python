...
from hltv_api.results import get_results, get_past_matches_ids
...
def test_get_matches_uri_work_for_single_result():
    query = HLTVQuery(start_date="1st Sep 2021", end_date="2nd Sep 2021")
    uris = get_past_matches_ids(limit=1, query=query)

    assert len(uris) == 1
    assert uris[0] == "2350368"


def test_get_matches_uri_work_for_multiple_results():
    query = HLTVQuery(start_date="1st Sep 2021", end_date="2nd Sep 2021")
    uris = get_past_matches_ids(limit=3, query=query)

    assert len(uris) == 3
    assert uris == ["2350368", "2351027", "2351022"]
