...
class HLTVScraper():
    ...
    def fetch_results(self, endpoint="/results", start_date=None, end_date=None, max_result_count=100):
        ...
        df = pd.DataFrame(columns=["event", "team_1", "team_2", "map", "score_1", "score_2"])

        while result_rem_count > 0 and new_row_count > 0:
            new_row_count, df = self.__fetch_results(df, endpoint, start_date, end_date, offset, result_rem_count)
            offset += new_row_count
            result_rem_count = min(max_result_count - offset, 100) 

        return df
            
    def __fetch_results(self, df, endpoint, start_date=None, end_date=None, offset=0, max_result_count=100):
